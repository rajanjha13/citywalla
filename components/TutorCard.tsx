...
const TutorCard: React.FC<TutorCardProps> = ({ tutor, onEnquiry }) => {
  return (
    <div 
      ...
      className="bg-[#0a0f1d] rounded-[32px] overflow-hidden shadow-2xl border border-white/5 flex flex-col group h-full cursor-pointer relative transition-all duration-500 hover:-translate-y-3"
    >
      <div className="relative h-60 md:h-64 overflow-hidden">
        ...
        <div className="absolute bottom-5 left-6 right-6">
           <div className="flex justify-between items-end">
              <div>
                 <div className="text-white text-2xl font-black tracking-tighter leading-none">₹{tutor.hourlyRate}<span className="text-xs opacity-60 font-medium tracking-normal">/hr</span></div>
              </div>
      ...
      <div className="p-6 flex-grow flex flex-col">
        <h3 className="text-xl font-black text-white tracking-tight mb-1.5 group-hover:text-indigo-400 transition-colors leading-tight">{tutor.name}</h3>
        <p className="text-xs text-slate-500 font-bold flex items-center gap-2 mb-5">
          <svg className="w-4 h-4 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path></svg>
          {tutor.location}
        </p>
