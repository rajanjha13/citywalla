...
const LoginModal: React.FC<LoginModalProps> = ({ onLogin, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-lg animate-in fade-in duration-300">
      <div className="w-full max-w-4xl h-auto max-h-[95vh] bg-white rounded-[40px] shadow-2xl flex flex-col md:flex-row overflow-hidden animate-in zoom-in-95 duration-500">
        
        {/* Left Branding Panel */}
        <div className="hidden md:flex flex-col justify-between w-2/5 bg-slate-900 p-12 text-white relative overflow-hidden flex-shrink-0">
        ...
        {/* Right Form Panel */}
        <div className="flex-1 p-6 sm:p-8 md:p-12 lg:p-16 flex flex-col justify-center relative overflow-y-auto">
          <button 
            onClick={onClose}
            className="absolute top-6 right-6 md:top-8 md:right-8 text-slate-300 hover:text-slate-900 transition-colors active:scale-90 z-20"
          >
          ...
          <div className="w-full max-w-md mx-auto">
            <div className="md:hidden flex items-center gap-2 mb-8">
              ...
              <span className="text-2xl font-black tracking-tighter text-slate-800 uppercase">CITYWALA</span>
            </div>

            <div className="mb-8">
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">{current.title}</h2>
              <p className="text-base sm:text-lg text-slate-500 font-medium mt-2">{current.desc}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input 
                  type="text" 
                  className="w-full px-5 py-4 sm:px-6 sm:py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition-all text-lg sm:text-xl font-bold text-slate-800 placeholder:text-slate-300"
                  value={userId}
              ...
              <div className="pt-3">
                <button 
                  type="submit" 
                  className="w-full py-5 sm:py-6 text-white bg-indigo-600 rounded-2xl font-black text-base uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
                >
                  {current.buttonText}
                </button>
              </div>
